...
from xdumbo.core.xdumbo_k import XDumbo_k
from xdumbo.core.xdumbo_k_s_nosig import XDumbo_k_s
...
class XDSNNode (XDumbo_k_s):
    ...
    def __init__(self, sid, id, S, T, Bfast, Bacs, N, f,
        ...
        self.stop = stop
        self.mode = mode
        XDumbo_k.__init__(self,sid, id, max(S, 10), max(int(Bfast), 1), N, f,
        ...
        if self.mode == 'test' or 'debug': #K * max(Bfast * S, Bacs)
            ...
            for k in range(self.K):
                for r in range(max(self.B * self.SLOTS_NUM, 1)):
                    ...
                    XDumbo_k.submit_tx(self, tx[:-len(suffix)] + suffix, k)
