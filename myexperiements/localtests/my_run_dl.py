...
def _test_dl(N=16, f=5, seed=None):
    sid = 'sidA'
    ...
    for i in range(N):
        dls[i] = DL(sid, i, B, N, f, sPK, sSKs[i], PK1, SK1s[i], PK2s, SK2s[i], sends[i], recvs[i], K)
        # print(sPK, sSKs[i], ePK, eSKs[i])

    for r in range(K * B):
        for i in range(N):
            # if i == 1: continue
            dls[i].submit_tx('<[Dummy TX %d]>' % (i + 10 * r))

    for i in range(N):
        threads[i] = gevent.spawn(dls[i].run_bft)
