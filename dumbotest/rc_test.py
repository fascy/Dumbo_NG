...
store = [0 for n in range(4)]
lock = [0 for n in range(4)]
done = [0 for n in range(4)]
PK, SKs = dealer(4, 3)
...
def _test_rc(store, lock, N=4, f=1, seed=None):
    sid = 'sidA'
    sends, recvs = simple_router(N, seed=seed)
    threads = []
    outputs = [Queue(1) for _ in range(N)]
    for i in range(N):
        t = Greenlet(recastsubprotocol, i, sid, N, f, PK, SKs[i], recvs[i], sends[i], store[i], lock[i])
        t.start()
        threads.append(t)
...
def test_pd(N, f, seed):
    ...
    _test_rc(store=store, lock=lock, N=4, f=1, seed=None)
